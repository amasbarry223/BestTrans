...
const inputClass =
  'w-full h-12 pl-11 pr-4 rounded-2xl border border-[#E5E7EB]/80 bg-[#F9FAFB]/90 text-[#111827] text-sm placeholder:text-[#9CA3AF] transition-all duration-200 focus:outline-none focus:bg-white focus:border-emerald-400/60 focus:ring-4 focus:ring-emerald-500/10'
...
export function LoginForm() {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error('Identifiants incorrects', {
          description: 'Vérifiez votre identifiant agent et votre mot de passe.',
        })
        return
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="relative group">
          <User
            className={cn(
              ...
              focused === 'id' ? 'text-emerald-500' : 'text-[#9CA3AF]'
            )}
          />
          <input
            id="identifier"
      ...
      <div className="space-y-2">
        ...
        <div className="relative group">
          <Lock
            className={cn(
              'absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors',
              focused === 'pwd' ? 'text-emerald-500' : 'text-[#9CA3AF]'
            )}
          />
          <input
            id="password"
            ...
            <span
              className={cn(
                ...
                remember
                  ...
                  : 'border-[#D1D5DB] group-hover:border-emerald-300'
              )}
            >
              <Check
                className={cn(
      ...
      <div className="rounded-2xl border border-dashed border-emerald-200/80 bg-emerald-50/50 px-4 py-3 mt-2">
        <p className="text-[11px] font-medium text-emerald-800/70 text-center mb-1.5">
          Accès démonstration
        </p>
        <div className="flex items-center justify-center gap-2 flex-wrap">
          <code className="text-xs font-mono bg-white/80 px-2.5 py-1 rounded-lg text-[#374151] border border-emerald-100">
            agent001
          </code>
