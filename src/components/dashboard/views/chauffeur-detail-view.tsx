...
const kycStatusStyles: Record<string, { bg: string; text: string; dot: string }> = {
  ...
  'En attente': { bg: 'bg-blue-100', text: 'text-blue-700', dot: 'bg-blue-500' },
                ...
                <div className="flex items-center gap-2 mt-auto">
                  {doc.status === 'En attente' && (
                    <>
                      <Button
                        size="sm"
                        className="bg-blue-600 hover:bg-blue-700 text-white text-xs gap-1 h-7 px-2.5 flex-1"
                        onClick={() =>
                  ...
                  {doc.status === 'Validé' && (
                    <Button
                      size="sm"
                      variant="ghost"
                      className="text-blue-600 hover:bg-blue-50 text-xs gap-1 h-7 px-2.5"
                    >
                      Voir document
                    </Button>
                  )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white border border-[#E5E7EB] rounded-xl p-5 flex items-center gap-4 hover:shadow-md transition-shadow">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-blue-700">125 000 FCFA</p>
          </div>
        </div>
        <div className="bg-white border border-[#E5E7EB] rounded-xl p-5 flex items-center gap-4 hover:shadow-md transition-shadow">
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-xl p-5 flex items-center gap-4 hover:shadow-md transition-shadow">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-blue-700">210 000 FCFA</p>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {defaultRecentCourses.map((course) => (
                <tr
                  key={course.id}
                  className="border-b border-[#F3F4F6] last:border-b-0 hover:bg-blue-50/30 transition-colors"
                >
                  <td className="py-2.5 px-5 text-xs font-mono text-blue-600">{course.id}</td>
        ...
        <div className="md:hidden divide-y divide-[#F3F4F6] max-h-[300px] overflow-y-auto">
          {defaultRecentCourses.map((course) => (
            <div key={course.id} className="px-4 py-3 flex flex-col gap-1.5">
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono text-blue-600">{course.id}</span>
                <span
                  className={cn(
                    'inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold',
                    course.statut === 'Terminée'
